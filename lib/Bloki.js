...
var Bloki = function (_Component) {
	...
	function Bloki() {
		...
		return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Bloki.__proto__ || Object.getPrototypeOf(Bloki)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
			...
			debug && (console.log(normalizedWidth), console.log('calc(' + 100 / columns * normalizedWidth + '% - ' + spacing + 'px)'));
			...
			if (col && width[theme.currentBreakpoint]) {
				return _this.getWidth(width[theme.currentBreakpoint], theme.columns, spacing); // this.width.xs.2
			} else if (row) {
				return '100%';
			} else {
				return auto || !col && !row ? null : _this.getWidth(theme.columns, theme.columns, spacing); // this.width.xs.2
			}
			...
			var styles = _extends({
				flexBasis: nest ? null : _this.columnWidth({ xs: xs, sm: sm, md: md, lg: lg, xl: xl }, theme, spacing),
				maxWidth: nest ? null : _this.columnWidth({ xs: xs, sm: sm, md: md, lg: lg, xl: xl }, theme, spacing),
				padding: padding,
				marginLeft: margin,
				marginRight: margin,
	...
	_createClass(Bloki, [{
		...
		value: function render() {
			...
			var _props2 = this.props,
			    ...
			    style = _props2.style,
			    debug = _props2.debug,
			    rest = _objectWithoutProperties(_props2, ['children', 'style', 'debug']);
			...
			return _react2.default.createElement(
				...
				function (theme) {
					...
					var styles = _this2.getStyles(theme);
					...
					return (
						...
						typeof children === 'function' ? _react2.default.createElement(
							'div',
							{
								style: _extends({}, styles, style, _this2.props[breakPointStyle], breakPointUpStyle)
							},
							children(theme)
